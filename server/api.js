...
import { consumeState, createScan, createSession, dbHealth, deleteSession, getScan, getSession, saveState, saveUser } from './db.js';
...
function session(req) { return getSession(cookies(req)[COOKIE]); }
...
export async function handleApiRequest(req, res) {
  ...
  if (url.pathname === '/api/analyze' && req.method === 'POST') {
    ...
    const id = await createScan(current.githubId); await enqueueScan(id); return json(res, 202, { id, status: 'queued', progress: 0 });
  }
